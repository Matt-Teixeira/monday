#!/usr/bin/env bash
# Release monday: mirror THIS working tree to /opt/apps/<APP_NAME>, apply the
# #RELEASE: .env overrides, stamp the released commit, build as svc.
# Fleet paradigm — adapted from data_acquisition's build-release.sh
...
#   5. chown + build as svc  — image becomes monday:svc
set -euo pipefail
...
# be copied by tar: files/ (export_csv CSVs), data_outputs/, notes/, ex-data/,
# and the root-level report outputs (*.csv, *.txt, acu-rtt-hhm-mapping-*.md,
# inspect.json). Patterns are deliberately NARROW: no tracked .csv/.txt/.crt
# exists (verified via git ls-files), and the tracked .md files (CLAUDE.md,
# PROCESS-FLOW.md) do not match the mapping-report glob. Verify exclude changes
# by diffing `tar -tf` output, never by eyeballing patterns.
sudo mkdir -p "$DEST"
...
sudo tar -C "$SRC" \
    ...
    --exclude='*.crt' \
    --exclude='*.log' \
    --exclude='./inspect.json' \
    --exclude='acu-rtt-hhm-mapping-*.md' \
    -cf - . | sudo tar -C "$DEST" -xf -

# export_csv writes ./files and dies without it; the dir is excluded from the
# mirror above (gitignored CSV bulk), so recreate it deliberately owned.
...
# index.js prints this on every boot line, so every run's console output (the
# cron .out files) records the commit that produced it. Decision 2026-08-25:
# no release_sha column on the shared stats.job_runs table — provenance lives
# here and on the boot line. Never set by hand; a dev tree has no key and
# prints 'dev-tree' instead.
...
echo "==> release complete: $DEST  commit: $GIT_SHA  image: monday:svc"
